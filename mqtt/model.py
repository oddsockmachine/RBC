...
def create_node(name, location, location_url, lat, lon, tags):
    uid = str(uuid4()).split("-")[0]
    n = Node(name=name, uid=uid, lat=lat, lon=lon, location=location, location_url=location_url, tags=tags).init()
    n.save()
    print(n)
    return n
...
def get_location_by_url(_url):
    ...
    if len(locs)==1:
        ...
        print("Location not found at url {}".format(url))
        return None
    else:
